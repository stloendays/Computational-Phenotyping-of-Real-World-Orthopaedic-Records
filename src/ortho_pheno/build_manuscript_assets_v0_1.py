...
def write_figure1(path: Path):
    ...
    for cx in (325,610,875):
        s += line(cx,520,cx,545)
...
def write_figure2(path: Path, cohort_rows, duplication_rows, anatomy_rows, transition_rows):
    ...
    retained=sum(int(r['n']) for r in transition_rows if r['previous_rule_label']=='old_wrist_scaphoid' and r['v0_3_anatomy_class']=='wrist_scaphoid')
    removed=sum(int(r['n']) for r in transition_rows if r['previous_rule_label']=='old_wrist_scaphoid' and r['v0_3_anatomy_class']=='foot_navicular')
    additions=sum(int(r['n']) for r in transition_rows if r['previous_rule_label']=='old_not_wrist' and r['v0_3_anatomy_class']=='wrist_scaphoid')
    ...
    s += text(992,558,f"{previous} episodes","small","middle")
    s += rect(1250,485,245,70,"dark")
    s += text(1372,516,"Retained wrist scaphoid","label white","middle")
    s += text(1372,539,f"{retained} episodes","small white","middle")
    s += rect(1250,595,245,70,"soft")
    s += text(1372,626,"Removed as foot navicular","label","middle")
    s += text(1372,649,f"{removed} episodes","small","middle")
    s += arrow(1115,530,1240,520)
    s += arrow(1115,548,1240,625)
    s += text(1170,505,str(retained),"small","middle")
    s += text(1170,610,str(removed),"small","middle")
    s += text(875,720,f"New wrist additions relative to the superseded heuristic: {additions}", "small")
...
def write_figure3(path: Path, relevance_rows):
    ...
    for d in ('hallux_valgus','scaphoid_fracture','first_cmc_oa'):
        ...
        if mod>con:
            s += text(830,y+35,f"{mod-con} admission(s) not disease-concordant", "tiny")
        y += 92
    ...
    boxes=[
        ...
        (995,565,500,66,"2. Components conditional on gold relevance=yes","Extraction after attribution is known"),
        (995,660,500,66,"3. End-to-end components","Relevance and component errors both count"),
    ]
    ...
    s += text(995,785,"This prevents high component F1 from hiding wrong-disease attribution.","small")
...
def write_table1(csv_path: Path, md_path: Path, cohort_rows, duplication_rows, relevance_rows):
    ...
    header=['Domain','Candidates','Strict phenotype*','Dx rows/admission','Female, %','Age, median [IQR]','BMI, median','Complaint n','Physical exam n','Detailed note module n','Target-disease procedure note n','Imaging/exam record n','CBC n','Coagulation n']
    md=['# Table 1. Source architecture and deterministic cohort reconstruction\n']
